...
export class Options {
    ...
    static init() {
        for (let i in Options.opts) {
            Options.addOption(i, this.opts[i]);
        }
    }
    ...
    static opts: any = {
        ...
        28: {
            ...
            default: function (server: DhcpServer) {
                ...
                const cidr = Tools.CIDRFromNetmask(server.config('netmask'));
    ...
    static addOption(code: any, opt: any) {
        Options.opts[code] = opt;

        if (opt.config) {
            Options.conf[opt.config] = parseInt(code, 10);
        } else if (opt.attr) {
            Options.conf[opt.attr] = parseInt(code, 10);
        }
    }
}